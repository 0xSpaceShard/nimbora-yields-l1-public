...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const addresses =
    network == "mainnet"
      ...
      : networkAddresses["goerli"];

  const balance = await hre.ethers.provider.getBalance(deployer);
  console.log("deployer", deployer);
  console.log("balance", balance);

  const starknetPoolingManagerDeployment = await deploy(
  ...
  console.log(
    `Pooling manager contract deployed at ${starknetPoolingManagerDeployment.address}`,
  );
};
