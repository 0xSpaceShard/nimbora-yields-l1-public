...
describe("StrategyBase Test", function () {
  async function loadFixture() {
    ...
    const testStrategyBaseFactory =
      await ethers.getContractFactory("TestStrategyBase");
    const yield_factor = ethers.parseUnits("0.1", "ether");
    const testStrategyBase = await upgrades.deployProxy(
      testStrategyBaseFactory,
      [
  ...
  it("test init", async function () {
    ...
    expect(await testStrategyBase.poolingManager()).equal(
      poolingManagerAddress,
    );
    expect(await testStrategyBase.underlyingToken()).equal(
      underlyingTokenAddress,
    );
  ...
  it("test handleReport revert invalid caller", async function () {
    ...
    await expect(
      testStrategyBase.handleReport(actionId, amount),
