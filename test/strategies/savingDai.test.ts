...
describe("Strategy SavingDai Test", function () {
  ...
  it("Should init the strategy ", async function () {
    ...
    expect(await sdaiStrategy.poolingManager()).eq(
      starknetPoolingManagerAddress,
    );
  });
  ...
  it("Should return the depositCalldata ", async function () {
    ...
    const encodedData = iface.encodeFunctionData("deposit(uint256,address)", [
      amount,
      sdaiStrategyAddress,
    ]);
  ...
  it("Should deposit into the strategy", async function () {
    ...
    await dai
      ...
      .approve(await sdai.getAddress(), amount);
    const { target, cdata } = await sdaiStrategy
      .connect(poolingManagerSigner)
      .depositCalldata(amount);

    const pmBalancesDaiBefore = await sdai.balanceOf(
      starknetPoolingManagerAddress,
    );
    ...
    const pmBalancesDaiAfter = await sdai.balanceOf(
      starknetPoolingManagerAddress,
    );
  ...
  it("test acion withdraw enough yield balance", async function () {
    ...
    let balanceUnderlyingBefore = await dai.balanceOf(
      starknetPoolingManagerAddress,
    );

    // Withdraw
    await sdaiStrategy.connect(poolingManagerSigner).withdraw(withdrawalAmount);
    let balanceUnderlyingAfter = await dai.balanceOf(
      starknetPoolingManagerAddress,
    );
    const balanceYieldAfter = await sdai.balanceOf(sdaiStrategyAddress);
    let expectedUnderlying =
      await sdaiStrategy.yieldToUnderlying(withdrawalAmount);
    const expectedYield =
      await sdaiStrategy.yieldToUnderlying(withdrawalAmount);

    expect(balanceYieldBefore - balanceYieldAfter).eq(expectedYield);
    expect(balanceUnderlyingAfter - balanceUnderlyingBefore).eq(
      expectedUnderlying,
    );
    ...
    // Withdraw
    balanceUnderlyingBefore = await dai.balanceOf(
      starknetPoolingManagerAddress,
    );
    ...
    expect(balanceUnderlyingAfter - balanceUnderlyingBefore).equal(
      withdrawalAmount,
    );
  });
  ...
  it("Should withdraw when not enough yield balance", async function () {
    ...
    expect(await sdai.balanceOf(sdaiStrategyAddress)).eq(
      await sdaiStrategy.yieldToUnderlying(underlyingAmount),
    );
    ...
    expect(await dai.balanceOf(starknetPoolingManagerAddress)).eq(
      underlyingAmount,
    );
  });
  ...
  it("Should fail if the caller is not admin", async function () {
    ...
    await expect(
      sdaiStrategy.connect(user).upgradeToAndCall(randomAddress, "0x"),
    ).revertedWithCustomError(sdaiStrategy, "CallerIsNotAdmin");
  });

  it("Should fail if the caller is pooling manager", async function () {
    const amount = ethers.parseUnits("10", "ether");
    await expect(
      sdaiStrategy.connect(user).withdraw(amount),
    ).revertedWithCustomError(sdaiStrategy, "CallerIsNotPoolingManager");
  });
});
