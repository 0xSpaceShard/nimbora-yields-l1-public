...
describe('Strategy UniswapV3 Test', function () {
    ...
    beforeEach('Strategy UniswapV3 Test', async () => {
        ...
        mockV3Aggregator = await mockV3AggregatorFactory.deploy(18, '200000000000000000');
    ...
    it('Should initilaze the strategy', async function () {
        ...
        expect(await uniswapV3Strategy.minReceivedAmountFactor()).equal(minReceivedAmountFactor);
        expect(await uniswapV3Strategy.poolFee()).equal(poolFee);
    });

    it('Should be able to return chainlinkLatestAnswer', async function () {
        expect(await uniswapV3Strategy.chainlinkLatestAnswer()).eq('200000000000000000');
    });

    it('Should be able to return addressToApprove', async function () {
        expect(await uniswapV3Strategy.addressToApprove()).eq(uniswapRouterMockAddress);
    });
    ...
    it('Should not be able to setMinReceivedAmountFactor, not admin caller', async function () {
        const minFact = '980000000000000000';
        await expect(uniswapV3Strategy.connect(user).setMinReceivedAmountFactor(minFact)).revertedWithCustomError(
            uniswapV3Strategy,
            'CallerIsNotAdmin',
        );
    });
    ...
    it('Should fail setMinReceivedAmountFactor', async function () {
        ...
        await expect(uniswapV3Strategy.setMinReceivedAmountFactor(fact)).revertedWithCustomError(
            uniswapV3Strategy,
            'InvalidSlippage',
        );
    ...
    it('Should deposit into the strategy', async function () {
        ...
        await weth.connect(poolingManagerSigner).approve(uniswapRouterMockAddress, amount);
        const { target, cdata } = await uniswapV3Strategy.connect(poolingManagerSigner).depositCalldata(amount);

        const balancewstEthBefore = await wstEth.balanceOf(uniswapV3StrategyAddress);
        const balancewEthBefore = await weth.balanceOf(starknetPoolingManagerAddress);
        ...
        const balancewEthAfter = await weth.balanceOf(starknetPoolingManagerAddress);
    ...
    it('Should withdraw from strategy, many times', async function () {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const wstEthBefore = await wstEth.balanceOf(uniswapV3StrategyAddress);
            const wethBefore = await weth.balanceOf(starknetPoolingManagerAddress);
            const expectedYield = await uniswapV3Strategy.underlyingToYield(amountWithdraw);

            await uniswapV3Strategy.connect(poolingManagerSigner).withdraw(amountWithdraw);
