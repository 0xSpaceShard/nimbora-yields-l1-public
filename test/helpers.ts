...
export function computeMessageReceivedL1(
  l2Pooling: BigNumberish,
  l1Pooling: BigNumberish,
  hash: bigint,
) {
  const encoder = ethers.AbiCoder.defaultAbiCoder();
  let encodedData = "0x";
...
export function computeMessageReceivedL2(
  l1Pooling: any,
  l2Pooling: any,
  nonceMessaging: any,
  epoch: any,
...
export function computeFromReportL1(
  strategyReportL1: PoolingManagerBase.StrategyReportStructOutput[],
) {
  const encoder = ethers.AbiCoder.defaultAbiCoder();
  let encodedData = "0x";
