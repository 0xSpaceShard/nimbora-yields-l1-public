...
function computeFromReportL2(
  new_epoch: any,
  bridgeInfo: any,
  strategyReportL2: any,
  bridgeDepositInfo: any,
) {
  ...
  for (let index = 0; index < bridgeInfo.length; index++) {
    ...
    encodedData += encoder
      .encode(
        ["uint256", "uint256"],
        [bridgeInfoElement.bridge, bridgeInfoElement.amount],
      )
      .slice(2);
  }
    ...
    encodedData += encoder
      .encode(
        ...
        [
          ...
          strategyReportL2Element.actionId,
  ...
  for (let index = 0; index < bridgeDepositInfo.length; index++) {
    ...
    encodedData += encoder
      .encode(
        ["uint256", "uint256"],
        [bridgeDepositInfoElement.bridge, bridgeDepositInfoElement.amount],
      )
      .slice(2);
  }
...
function computeMessageReceivedL1(
  l2Pooling: any,
  l1Pooling: any,
  hash: bigint,
) {
...
function computeMessageReceivedL2(
  l1Pooling: any,
  l2Pooling: any,
  nonceMessaging: any,
  epoch: any,
  ...
  const { lowPart: lowEpoch, highPart: highEpoch } =
    splitUint256ToUint128(epoch);
  ...
  for (let index = 0; index < strategyReportL1.length; index++) {
    ...
    encodedData += encoder
      .encode(
        ["uint256", "uint256", "uint256"],
        [
          strategyReportL1Element.l1Strategy,
          strategyReportL1Element.l1Nav,
          strategyReportL1Element.amount,
        ],
      )
      .slice(2);
  }
...
describe("Pooling Manager Test", function () {
  async function InitPoolingManager() {
    ...
    const poolingManagerFactory =
      await ethers.getContractFactory("PoolingManager");
  ...
  async function InitUniswap(
    ownerAddress: string,
    poolingManagerAddress: string,
    weth: WETH9,
    wethAddress: string,
    wsteth: ERC20Mock,
    ...
    uniswapRouterMock.setExchangeRate(
      wstethAddress,
      wethAddress,
      "1200000000000000000",
    );
    ...
    const uniswapV3Strategy = await upgrades.deployProxy(
  ...
  async function InitSavingDai(
    ...
    const savingDaiStrategy = await upgrades.deployProxy(
      savingDaiStrategyFactory,
      [poolingManagerAddress, daiAddress, sdaiAddress],
    );
  ...
  it("test init", async function () {
    ...
    expect(await poolingManager.hasRole(RELAYER_ROLE, relayer.address)).equal(
      true,
    );
  ...
  it("should return correct hash for single element in each array", async function () {
    ...
    const strategyReportL2 = [
      { l1Strategy: addressUsed, actionId: "2", amount: "8" },
    ];
    const bridgeDepositInfo = [{ bridge: addressUsed, amount: "4" }];
    const epoch = 0;
    const expectedHashForSingleElements = computeFromReportL2(
      epoch,
      bridgeInfo,
      strategyReportL2,
      bridgeDepositInfo,
    );
    const hash = await poolingManager.hashFromReportL2(
      epoch,
      bridgeInfo,
      strategyReportL2,
      bridgeDepositInfo,
  ...
  it("should return correct hash for diff elements in each array", async function () {
    ...
    const bridgeInfo = [{ bridge: addressUsed1, amount: "3" }];
    const strategyReportL2 = [
    ...
    const epoch = 0;
    const expectedHashForDiffElements = computeFromReportL2(
      epoch,
      bridgeInfo,
      strategyReportL2,
      bridgeDepositInfo,
  ...
  it("should return correct hash for single element in each array", async function () {
    ...
    const strategyReportL1 = [
      { l1Strategy: addressUsed, l1Nav: "2", amount: "244" },
    ];
  ...
  it("test registerStrategy revert zero value strategy", async function () {
    ...
    await expect(
      poolingManager.registerStrategy(
        zeroAddress,
        wethAddress,
        ethBridgeAddress,
      ),
    ).to.be.revertedWithCustomError(poolingManager, "ZeroAddress");
  });
  ...
  it("test registerStrategy revert InvalidPoolingManager", async function () {
    ...
    ).to.be.revertedWithCustomError(poolingManager, "InvalidPoolingManager");
  });
  ...
  it("test registerStrategy", async function () {
    ...
    await poolingManager.registerStrategy(
      uniswapV3StrategyAddress,
      wethAddress,
      ethBridgeAddress,
    );
    expect(
      await weth.allowance(poolingManagerAddress, ethBridgeAddress),
    ).to.be.equal(maxUint256);
    const strategyInfoUni = await poolingManager.strategyInfo(
      uniswapV3StrategyAddress,
    );
    expect(strategyInfoUni.underlying).to.be.equal(weth);
    expect(strategyInfoUni.bridge).to.be.equal(ethBridgeAddress);

    const { savingDaiStrategy, savingDaiStrategyAddress } = await InitSavingDai(
      poolingManagerAddress,
      daiAddress,
      sdaiAddress,
    );
    await poolingManager.registerStrategy(
      savingDaiStrategyAddress,
      daiAddress,
      daiBridgeAddress,
    );
    expect(
      await dai.allowance(poolingManagerAddress, daiBridgeAddress),
    ).to.be.equal(maxUint256);
    const strategyInfoSdai = await poolingManager.strategyInfo(
      savingDaiStrategyAddress,
    );
  ...
  it("test handleReport revert invalid message to consume", async function () {
    ...
    const bridgeInfo = [{ bridge: ethBridgeAddress, amount: depositAmount }];
    const bridgeInfoFake = [
      { bridge: Wallet.createRandom().address, amount: depositAmount },
    ];
    const strategyReportL2 = [
      {
        l1Strategy: uniswapV3StrategyAddress,
        actionId: "0",
        amount: depositAmount,
      },
    ];
    const bridgeDepositInfo: any = [];
    const expectedHash = computeFromReportL2(
      epoch,
      bridgeInfoFake,
      strategyReportL2,
      bridgeDepositInfo,
  ...
  it("test handleReport one strategy with deposit order, no revert, no pending", async function () {
    ...
    const bridgeDepositInfo: any = [];
    const expectedHashL2 = computeFromReportL2(
      epoch,
      bridgeInfo,
      strategyReportL2,
      bridgeDepositInfo,
    ...
    await expect(
      poolingManager.handleReport(
        ...
        l2MessagingEthFee,
        1,
        { value: l2MessagingEthFee },
      ),
    ).to.emit(poolingManager, "ReportHandled");

    // Make sure the right message has been sent
    const expectedReport = [
      {
        l1Strategy: uniswapV3StrategyAddress,
        l1Nav: "999999999999999999",
        amount: "0",
      },
    ];
    const expectedHashL1 = computeFromReportL1(expectedReport);
    const nonceMessaging = await starknetMock.l1ToL2MessageNonce();
    const messageReceivedL2 = computeMessageReceivedL2(
      poolingManagerAddress,
      l2PoolingManager,
      (nonceMessaging - BigInt(1)).toString(),
      BigInt(epoch),
      BigInt(expectedHashL1),
    );
    const { lowPart: lowEpoch, highPart: highEpoch } = splitUint256ToUint128(
      BigInt(epoch),
    );
    const { lowPart: lowHash, highPart: highHash } = splitUint256ToUint128(
      BigInt(expectedHashL1),
    );
    const messageReceivedL2Call = await starknetMock.getL1ToL2MsgHash(
      poolingManagerAddress,
      l2PoolingManager,
      L2_HANDLER_SELECTOR,
      [lowEpoch, highEpoch, lowHash, highHash],
      nonceMessaging - BigInt(1),
    );
  ...
  it("test handleReport one strategy with report order, no revert, no pending", async function () {
    ...
    let epoch = "1";
    const bridgeInfo = [{ bridge: ethBridgeAddress, amount: BigInt(0) }];
    const strategyReportL2 = [
      {
        l1Strategy: uniswapV3StrategyAddress,
        actionId: "1",
  ...
  it("test handleReport one strategy with deposit order, revert, no pending", async function () {
    ...
    ).to.revertedWithCustomError(poolingManager, "NoL1Report");
  });
  ...
  it("test handleReport two strategy with deposit order, revert, no pending", async function () {
    ...
    const strategyReportL2 = [
      ...
      {
        l1Strategy: savingDaiStrategyAddress,
        actionId: "0",
        amount: depositAmountDai,
      },
    ];
    const bridgeDepositInfo: any = [];
    const expectedHashL2 = computeFromReportL2(
    ...
    ).to.revertedWithCustomError(poolingManager, "NotEnoughSuccessCalls");
  });
