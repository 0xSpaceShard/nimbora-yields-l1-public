...
import {
  testCasesReportsMock1,
  testCasesReportsMock2,
  testCasesReportsMock3,
} from "./__mocks__/reports";
...
describe("Starknet Pooling Manager Test", function () {
  ...
  beforeEach(async () => {
    ...
    // DAI token mock
    const erc20MintableMockFactory =
    ...
    // Starknet dai bridge
    const starkneErc20BridgeMockFactory = await ethers.getContractFactory(
      "StarknetErc20BridgeMock",
    );
    daiBridge = await starkneErc20BridgeMockFactory.deploy(
      await dai.getAddress(),
    );
    daiBridgeAddress = await daiBridge.getAddress();

    // sDAI Strategy
    const savingDaiStrategyFactory =
      await ethers.getContractFactory("SavingDaiStrategy");
    ...
    sdaiStrategy = SavingDaiStrategy__factory.connect(
  ...
  it("Should register strategy", async function () {
    await starknetPoolingManager.registerStrategy(
      await sdaiStrategy.getAddress(),
    );
    ...
    expect(allowance).eq(ethers.MaxUint256);
    const filter =
      starknetPoolingManager.filters["StrategyRegistered(address)"];
  ...
  it("Should fail to register strategy: invalid strategy contract", async function () {
    ...
    await expect(starknetPoolingManager.registerStrategy(randomAddress))
      .reverted;
  });
  ...
  it("Should fail to register strategy: not admin", async function () {
    await expect(
      starknetPoolingManager
        .connect(user)
        .registerStrategy(await sdaiStrategy.getAddress()),
  ...
  it("Should success handle report, deposit/withdraw into a single strategy (sdai), simulate loose tokens", async function () {
    ...
    const testCases: HandleReportInput[] = testCasesReportsMock1(
      bridgeAddress,
      l1strategyAddress,
    );
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      if (testCase.strategyReportL2[0].data == Action.DEPOSIT) {
        ...
        const afterSdaiBalance = BigInt(
          await sdai.balanceOf(l1strategyAddress),
        );
        const expectedShares = BigInt(
          await sdaiStrategy.underlyingToYield(amount),
        );
        expect(afterSdaiBalance - beforeSdaiBalance).equal(expectedShares);

        const daiTokens = BigInt(
          await sdaiStrategy.yieldToUnderlying(afterSdaiBalance),
        );
        ...
        // Vaidations
        const amount = testCase.strategyReportL2[0].amount;
        const afterBridgeDaiBalance = BigInt(
          await dai.balanceOf(bridgeAddress),
        );
        const expectedDai = BigInt(
          await sdaiStrategy.yieldToUnderlying(amount),
        );
        expect(afterBridgeDaiBalance - beforeBridgeDaiBalance).equal(
          expectedDai - loosedAmount,
        );
  ...
  it("Should success handle report, deposit/withdraw into a single strategy (uniswap)", async function () {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      const beforeYieldBalance = BigInt(
        await wstEth.balanceOf(uniswapV3StrategyAddress),
      );
      const beforeBridgeUnderlayingBalance = BigInt(
        await admin.provider.getBalance(bridgeAddress),
      );
      ...
      if (testCase.strategyReportL2[0].data == Action.DEPOSIT) {
        ...
        // Vaidations
        const afterYieldBalance = BigInt(
          await wstEth.balanceOf(uniswapV3StrategyAddress),
        );
        const expectedShares = BigInt(
          await uniswapV3Strategy.underlyingToYield(amount),
        );
        expect(afterYieldBalance - beforeYieldBalance).equal(expectedShares);

        const afterBridgeUnderlayingBalance = BigInt(
          await admin.provider.getBalance(bridgeAddress),
        );
        expect(
          beforeBridgeUnderlayingBalance - afterBridgeUnderlayingBalance,
        ).equal(amount);
        ...
        expect(
          afterBridgeUnderlayingBalance -
            beforeBridgeUnderlayingBalance -
            BigInt(amount),
        ).eq(1);
  ...
  it("Should success handle report, deposit/withdraw into one strategy (sdai) when strategy reverted when withdraw", async function () {
    ...
    const testCases: HandleReportInput[] = testCasesReportsMock3(
      daiBridgeAddress,
      sdaiStrategyAddress,
    );
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      if (testCase.strategyReportL2[0].data == Action.DEPOSIT) {
        ...
        const amount = testCase.strategyReportL2[0].amount;
        const afterSdaiBalance = BigInt(
          await sdai.balanceOf(sdaiStrategyAddress),
        );
        const expectedShares = BigInt(
          await sdaiStrategy.underlyingToYield(amount),
        );
  ...
  it("Should success handle report, deposit/withdraw into one strategy (sdai) when strategy reverted when deposit", async function () {
    ...
    // Do only a deposit
    const testCases: HandleReportInput[] = [
      testCasesReportsMock3(daiBridgeAddress, sdaiStrategyAddress)[0],
    ];
  ...
  it("Should success handle report, deposit/withdraw into one strategy (uniswap) when strategy reverted when deposit", async function () {
    ...
    // Do only a deposit
    const testCases: HandleReportInput[] = [
      testCasesReportsMock3(ethBridgeAddress, uniswapV3StrategyAddress)[0],
    ];
  ...
  it("Should success handle report, deposit/withdraw into multiple strategies (sdai, uniswapV3)", async function () {
    ...
    for (let j = 0; j < strategies.length; j++) {
      const { yielToken, underlying, strategyAddress, bridge, isETH } =
        strategies[j];
      beforeBalances.push({
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      for (let j = 0; j < strategies.length; j++) {
        ...
        const { beforeBridgeUnderlayingBalance, beforeYieldBalance } =
          beforeBalances[j];
        ...
        if (testCase.strategyReportL2[0].data == Action.DEPOSIT) {
          ...
          const amount = testCase.strategyReportL2[0].amount;
          const afterYieldBalance = BigInt(
            await yielToken.balanceOf(strategyAddress),
          );
          const expectedShares = BigInt(
            await strategy.underlyingToYield(amount),
          );
          expect(afterYieldBalance - beforeYieldBalance).equal(expectedShares);

          const daiTokens = BigInt(
            await strategy.yieldToUnderlying(afterYieldBalance),
          );
          expect(daiTokens).equal(BigInt(amount));
          ...
          const filter = starknetPoolingManager.filters.ReportHandled;
          const event = (
            await starknetPoolingManager.queryFilter(filter, -1)
          )[0];
          expect(event.args.epoch).eq(testCase.epoch);
        } else if (testCase.strategyReportL2[0].data == Action.WITHDRAW) {
          // ****************************************
  ...
  it("Should initilize the pooling manager", async function () {
    ...
    expect(await starknetPoolingManager.l2PoolingManager()).equal(
      l2PoolingManager,
    );
    expect(
      await starknetPoolingManager.hasRole(adminRole, admin.address),
    ).equal(true);
    expect(
      await starknetPoolingManager.hasRole(relayerRole, relayer.address),
    ).equal(true);
    expect(await starknetPoolingManager.starknetCore()).equal(
      await starknetMock.getAddress(),
    );
    expect(await starknetPoolingManager.ethBridge()).equal(
  ...
  it("Should be able to call cancelDepositRequestBridge", async function () {
    ...
    const nonce = "1";
    await starknetPoolingManager.cancelDepositRequestBridge(
      await daiBridge.getAddress(),
      amount,
      nonce,
    );
  ...
  it("should return correct report hash", async function () {
    ...
    for (let i = 0; i < testCases.length; i++) {
      const { epoch, bridgeDepositInfo, bridgeWithdrawInfo, strategyReportL2 } =
        testCases[i];
  ...
  it("Should register new strategy", async function () {
    ...
    expect(
      await dai.allowance(await starknetPoolingManager.getAddress(), bridge),
    ).not.eq(0);
    expect(
      await dai.allowance(
        await starknetPoolingManager.getAddress(),
  ...
  it("Should fail to register new strategy, invalid contract implementation", async function () {
    ...
    await expect(starknetPoolingManager.registerStrategy(randomAddress1))
      .reverted;
  });
  ...
  it("Should fail to register new strategy, InvalidPoolingManager", async function () {
    ...
    const invalidSdaiStrategy = SavingDaiStrategy__factory.connect(
      await proxy2.getAddress(),
      admin,
    );
    await expect(
      starknetPoolingManager.registerStrategy(
        await invalidSdaiStrategy.getAddress(),
      ),
  ...
  it("Should return a valid l1->l2 hash", async function () {
    ...
    const l2Hash =
      9394751887553205148934003660731450063757890657721159828438100138263725105979n;
  ...
  it("Should return a valid l2->l1 hash", async function () {
    ...
    const l2Pooling = BigInt(
      "0x065a953f89a314a427e960114c4b9bb83e0e4195f801f12c25e4a323a76da0a9",
    );
    const l2Hash = BigInt(
      "0xFB53439B1CF941DF70B022AF04C0146AC03A05B429E84FE6C28CF6F5A3145CE1",
    );
    ...
    const l1Hash = await starknetPoolingManager.hashFromReport(
      5n,
      bridgeDeposit,
      data,
      [],
