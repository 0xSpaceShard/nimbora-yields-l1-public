...
async function main() {
  ...
  async function getMessageBridgeGasFee(
    ...
    const gasCost = await estimateL2GasCost(
      l1Address,
      l2Address,
      "handle_response",
      [uEpoch.low, uEpoch.high, uHash.low, uHash.high],
    );
  ...
  async function getEthBridgeGasFee(
    receiver: string,
    amount: BigNumber,
  ): Promise<BigNumber> {
  ...
  const parsedBridgeDeposits: PoolingManagerBase.BridgeDataStruct[] = [
    bridgeDeposit,
  ];
  ...
  const parsedDataStrategy: PoolingManagerBase.StrategyReportStruct[] = [
    dataStrategy1,
    dataStrategy2,
  ];
  ...
  if (parsedBridgeWithdrawals.length > 0) {
    l2BridgeEthFee = await getEthBridgeGasFee(
